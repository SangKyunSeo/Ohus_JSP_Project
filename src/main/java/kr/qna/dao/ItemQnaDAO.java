...
public class ItemQnaDAO {
	...
	//상품번호로 목록 가져오기
	public List<ItemQnaVO> getListQnaByItem_num (int item_num) throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<ItemQnaVO> list = null;
		...
		try {
			...
			sql = "SELECT * FROM item_qna q JOIN omember m USING(mem_num) "
					+ "JOIN item i USING(item_num) "
					+ "WHERE item_num=? ORDER BY q.qna_num DESC";
